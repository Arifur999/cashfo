  ...
  return (
    <div className="rounded-2xl bg-surface p-5 shadow-sm shadow-black/5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-neutral-900">Income vs Savings</h2>
        <div className="flex items-center gap-4 text-xs text-neutral-500">
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-brand-dark" /> Income
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-brand-primary/50" /> Savings
          </span>
        </div>
      </div>
              ...
              return (
                <div key={p.month} className="flex flex-1 flex-col items-center justify-end gap-1.5">
                  <div className="flex items-end gap-1" style={{ height: BAR_MAX_HEIGHT }}>
                    <div
                      title={`Income: ${formatCurrency(p.income, currency)}`}
                      className="w-4 rounded-t-sm bg-brand-dark sm:w-6"
                      style={{ height: Math.max(2, incomeHeight) }}
                    />
                    <div
                      title={`Savings: ${formatCurrency(p.savings, currency)}`}
