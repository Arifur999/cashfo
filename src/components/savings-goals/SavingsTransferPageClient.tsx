  ...
  return (
    <div className="h-full bg-brand-content px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Savings Transfer</h1>
          <p className="mt-1 text-sm text-neutral-500">Move saved money between goals.</p>
        </div>
        {canManage && (
          <button
            type="button"
      ...
      {goals.length < 2 && <p className="mb-4 text-sm text-neutral-400">You need at least two savings goals to transfer between them.</p>}

      <div className="overflow-hidden rounded-2xl bg-surface shadow-sm shadow-black/5">
        <div className="border-b border-neutral-100 px-4 py-3">
          <h2 className="text-sm font-semibold text-neutral-900">Transfer History</h2>
        </div>
              ...
              <tbody className="divide-y divide-neutral-50">
                {transfers.map((t, index) => (
                  <tr key={t.id}>
                    ...
                    <td className="px-4 py-3 text-neutral-600">{new Date(t.date).toLocaleDateString()}</td>
                    <td className="px-4 py-3 font-medium text-brand-danger">{t.fromGoalName}</td>
                    <td className="px-4 py-3 text-neutral-300">
                      <ArrowRight className="h-4 w-4" />
                    </td>
                    <td className="px-4 py-3 font-medium text-brand-primary">{t.toGoalName}</td>
                    <td className="px-4 py-3 text-right font-semibold tabular-nums text-neutral-900">{formatCurrency(t.amount, currency)}</td>
                    <td className="px-4 py-3 text-neutral-400">{t.notes ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
