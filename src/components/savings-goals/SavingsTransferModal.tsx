...
export function SavingsTransferModal({ open, onClose, businessId, goals }: SavingsTransferModalProps) {
  ...
  function handleSubmit() {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(result.message ?? t("Failed to transfer"));
      }
    });
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title={t("Savings Transfer")}>
      <div className="space-y-4">
        <div>
          <label className="mb-1 block text-sm font-medium text-neutral-700">{t("From Goal")}</label>
          <Combobox
            key={`from-${resetCount}`}
            value={fromGoalId}
            onChange={setFromGoalId}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-neutral-700">{t("To Goal")}</label>
          <Combobox
            key={`to-${resetCount}`}
            value={toGoalId}
        ...
        <p className="text-xs text-neutral-400">{t("Moves saved money between two goals. Your total savings stays the same.")}</p>
      </div>
