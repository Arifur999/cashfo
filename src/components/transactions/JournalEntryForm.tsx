...
export function JournalEntryForm({ businessId, accounts }: JournalEntryFormProps) {
  ...
  return (
    <div className="space-y-5 rounded-2xl bg-white p-6 shadow-sm shadow-black/5">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="mb-1 block text-sm font-medium text-neutral-700">Transaction Type</label>
          <select
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium text-neutral-700">Entries</p>
        {rows.map((row, index) => (
      ...
      <div className="flex items-center justify-between rounded-xl bg-neutral-50 px-4 py-3 text-sm">
        <span>
          Debits: <span className="font-medium tabular-nums">{debitTotal.toFixed(2)}</span> · Credits:{" "}
          <span className="font-medium tabular-nums">{creditTotal.toFixed(2)}</span>
        </span>
        {isBalanced ? (
          <span className="font-medium text-brand-primary">Balanced</span>
        ) : (
          <span className="font-medium text-brand-danger">Out of balance by {Math.abs(difference).toFixed(2)}</span>
        )}
      </div>
