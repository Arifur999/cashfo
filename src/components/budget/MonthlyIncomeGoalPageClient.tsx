...
export function MonthlyIncomeGoalPageClient({ businessId, goals, currency, canManage }: MonthlyIncomeGoalPageClientProps) {
  ...
  function handleDelete(goal: IncomeGoalSummary) {
    if (!window.confirm(`Delete the goal for ${MONTH_NAMES[goal.month - 1]} ${goal.year}?`)) return;
    ...
    startDeleteTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(result.message ?? "Failed to delete the income goal");
      }
      setDeletingId(null);
    });
  }
  ...
  return (
    <div className="h-full bg-brand-content px-6 py-8 pb-24 md:pb-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">Monthly Income Goal</h1>
          <p className="mt-1 text-sm text-neutral-500">Set an income target for any month and track how it&apos;s tracking against real income.</p>
        </div>
        {canManage && (
          <button
            type="button"
            ...
            {goals.length === 0 ? (
              <tr>
                <td colSpan={canManage ? 7 : 6} className="px-4 py-10 text-center text-sm text-neutral-400">
                  No goals set yet -- tap &quot;Add Goal&quot; to get started.
                </td>
              </tr>
            ) : (
