...
export function AssetValueHistoryModal({ open, onClose, asset, currency }: AssetValueHistoryModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={asset ? `${asset.name} -- Value History` : "Value History"}>
      {history.length === 0 ? (
        <p className="py-6 text-center text-sm text-neutral-400">No value history yet.</p>
      ) : (
