...
export function ReferralsPageClient({ businessId, info, currency, canManage }: ReferralsPageClientProps) {
  ...
  if (!info) {
    return (
      <div className="h-full bg-brand-content px-6 py-8">
        <p className="text-sm text-neutral-500">{t("Couldn't load your referral info -- please refresh.")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full bg-brand-content px-6 py-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-neutral-900">{t("Referral Program")}</h1>
        <p className="text-sm text-neutral-500">{t("Invite friends and earn rewards")}</p>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <div className="rounded-2xl bg-surface p-5 shadow-sm shadow-black/5">
            <h2 className="text-sm font-semibold text-neutral-900">{t("Your Referral Link")}</h2>
            <p className="mt-1 text-sm text-neutral-500">{t("Share this link with friends to earn rewards")}</p>
          ...
          <div className="rounded-2xl bg-surface p-5 shadow-sm shadow-black/5">
            <h2 className="text-sm font-semibold text-neutral-900">{t("How It Works")}</h2>
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <HowItWorksStep icon={UserPlus} title={t("Invite Friends")} description={t("Share your unique referral link with friends and family")} />
              <HowItWorksStep
                icon={CreditCard}
                title={t("They Sign Up")}
                description={t("When they create an account using your link, you both qualify for rewards")}
              />
              <HowItWorksStep
                icon={Gift}
                ...
                description={
                  <>
                    {t("You'll receive")} {formatCurrency(info.rewardAmountPerReferral, currency)}{" "}
                    {t("credit for each friend who signs up and stays active for 30 days")}
                  </>
                }
              />
            </div>
          ...
          <div className="rounded-2xl bg-surface p-5 shadow-sm shadow-black/5">
            <h2 className="text-sm font-semibold text-neutral-900">{t("Referral Program Terms")}</h2>
            <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-neutral-600">
              <li>{t("Referral rewards are credited once your friend has been registered for 30 days.")}</li>
              <li>{t("Your friend's account must still be active for the reward to count.")}</li>
              <li>{t("Rewards can be withdrawn to any of your General or Savings accounts.")}</li>
              <li>{t("There's no limit to how many friends you can refer.")}</li>
            </ol>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl bg-surface p-5 shadow-sm shadow-black/5">
            <h2 className="text-sm font-semibold text-neutral-900">{t("Earnings Overview")}</h2>

            <div className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-neutral-500">{t("Total Earned")}</span>
                <span className="font-semibold tabular-nums text-neutral-900">{formatCurrency(info.totalEarned, currency)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">{t("Pending Earnings")}</span>
                <span className="font-semibold tabular-nums text-neutral-900">{formatCurrency(info.pendingEarnings, currency)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">{t("Available Balance")}</span>
              ...
              <div className="flex justify-between border-t border-neutral-100 pt-2">
                <span className="text-neutral-500">{t("Referrals")}</span>
            ...
            <div className="mt-4 rounded-xl bg-brand-primary/5 p-4">
              <p className="text-xs font-medium text-neutral-500">{t("Available Balance")}</p>
                ...
                {info.recentReferrals.map((r) => (
                  <div key={r.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
                    ...
                    <div className="shrink-0 text-sm font-semibold text-neutral-900">
                      {r.status === "PENDING" ? <span className="text-neutral-400">{t("Pending")}</span> : `+${formatCurrency(r.rewardAmount, currency)}`}
                    </div>
                  </div>
                ))}
              </div>
